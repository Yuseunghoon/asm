...
void *reader(void *data_in)
{
    ...
    data->size = infoHeader.biSizeImage;    // 픽셀 데이터 크기
    data->width = infoHeader.biWidth;       // 비트맵 이미지의 가로 크기
    data->height = infoHeader.biHeight;     // 비트맵 이미지의 세로 크기
    ...
    data->padding = (PIXEL_ALIGN - ((data->width * PIXEL_SIZE) % PIXEL_ALIGN)) % PIXEL_ALIGN;

    if (data->size == 0)    // 픽셀 데이터 크기가 0이라면
    {
        // 이미지의 가로 크기 * 픽셀 크기에 남는 공간을 더해주면 완전한 가로 한 줄 크기가 나옴
        // 여기에 이미지의 세로 크기를 곱해주면 픽셀 데이터의 크기를 구할 수 있음
        data->size = (data->width * PIXEL_SIZE + data->padding) * data->height;
    }
    
    data->image = (unsigned char *)malloc(data->size);    // 픽셀 데이터의 크기만큼 동적 메모리 할당
    ...
    if (fread(data->image, data->size, 1, fpBmp) < 1)
    {
        fclose(fpBmp);
        return 0;
    }
...
void *writer(void *data_in)
{
    ...
    {
        free(data2->image);      // 픽셀 데이터를 저장한 동적 메모리 해제
        return 0;         // 프로그램 종료
    }

    // 픽셀 데이터는 아래 위가 뒤집혀서 저장되므로 아래쪽부터 반복
    // 세로 크기만큼 반복
    for (int y = data2->height - 1; y >= 0; y--)
    {
        // 가로 크기만큼 반복
        for (int x = 0; x < data2->width; x++)
        {
            ...
            int index = (x * PIXEL_SIZE) + (y * (data2->width * PIXEL_SIZE)) + (data2->padding * y);

            unsigned char *image2 = data2->image;

            // 현재 픽셀의 주소를 RGBTRIPLE 포인터로 변환하여 RGBTRIPLE 포인터에 저장
            RGBTRIPLE *pixel = (RGBTRIPLE *)&image2[index];
...
int main()
{
    ...
    im *data;
    data=(im *)malloc(sizeof(im));

    if((err=pthread_create(&p_thread[0],NULL, reader, (void*)data))<0)
    {
        perror("thread create 0 error\n");
        exit(1);
    }

    pthread_join(p_thread[0],(void **)&addr);

    if((err=pthread_create(&p_thread[1],NULL, writer, (void*)data))<0)
    {
        perror("thread create 0 error\n");
        exit(1);
    }

    pthread_join(p_thread[1],(void **)&addr);
